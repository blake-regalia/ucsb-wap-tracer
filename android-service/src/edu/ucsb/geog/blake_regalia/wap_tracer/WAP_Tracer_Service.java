...
public class WAP_Tracer_Service extends Service {
	...
	private final class ServiceHandler extends Handler {
		...
		private Runnable gps_fail = new Runnable() {
			public void run() {
				hardware_monitor.enable(HardwareMonitor.GPS, gps_ready, gps_fail, service_looper);
			}
		};
...
//					objective = WTO_START_GPS; break;
				...
				case WTO_START_GPS:
					...
					if(!hardware_monitor.enable(HardwareMonitor.GPS, gps_ready, gps_fail, service_looper)) {
						return;
					}
...
//					objective = WTO_START_WIFI; break;
				...
				case WTO_START_WIFI:
					...
					if(!hardware_monitor.enable(HardwareMonitor.WIFI, wifi_ready, wifi_fail, service_looper)) {
						return; 
					}
...
//					objective = WTO_SCAN_WIFI; break;
				...
				case WTO_WAKE_GPS:
					...
					locator.obtainLocation(LocationAdvisor.BOUNDARY_CHECK_LISTENER, location_fix, hw_fail, service_looper);
				...
				case WTO_CHECK_BOUNDARY:
					...
					if(location == null) {
						System.out.println("** location advisor timed out. failed to get a position fix");
				...
				case WTO_BEGIN_TRACE:
					debug("** starting scan");
					locator.obtainLocation(LocationAdvisor.TRACE_LOCATION_LISTENER, null, gps_position_lost, gps_fail_during_trace, service_looper);
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if(obj != null) {
			if(obj.equals(HardwareMonitor.ENABLE_GPS)) {
				objective = WTO_START_WIFI;
			}
			else if(obj.equals(HardwareMonitor.ENABLE_WIFI)) {
				objective = WTO_SCAN_WIFI;
			}
